...
import math
from .servo_345cv import Servo345cv
from json import dumps

@login_required
def servo345cv(request):
    ...
    if request.method == 'POST':
        ...
        if form_in.is_valid():
            ...
            pc_cv = float(form_in.cleaned_data['pc_cv'])
            ...
            s345cv = Servo345cv(s, t, pc_cv, d_pu, m, cof, f_res, j_pu, j_mot, j_gb, gr, t_idle)

            t_lst = s345cv.get_t_lst()

            s_lst = s345cv.get_s_lst()
            v_lst = s345cv.get_v_lst()
            a_lst = s345cv.get_a_lst()
            
            n_mot_lst = s345cv.get_n_mot_lst()  # list of motor rpm
            trq_mot_lst = s345cv.get_trq_mot_lst()

            t_mot_rms = s345cv.get_t_mot_rms()
            t_mot_pk = s345cv.get_t_mot_pk()
            n_mot_avg = s345cv.get_n_mot_avg()
            n_mot_pk = s345cv.get_n_mot_pk()
            a_pk = s345cv.get_a_pk()
            v_pk = s345cv.get_v_pk()
            t_acc = s345cv.get_t_acc()
            t_cv = s345cv.get_t_cv()
            s_acc = s345cv.get_s_acc()
            s_cv = s345cv.get_s_cv()
    ...
    return render(request, "servo_345cv_app/servo_345cv.html", 
        {
        'form_input': form_in,
