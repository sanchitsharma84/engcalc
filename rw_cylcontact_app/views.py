...
from .cylindrical_contact import CylindricalContact

import math

@login_required
def rw_cylcontact_fcn(request):
    form_in_lst = []
        ...
        for x in range(nr_rows):
            ...
            if this_form_in.is_valid():
                ...
                for x_key, y_value in this_form_in.cleaned_data.items():
                    if x_key != 'b' and x_key != 'p':  # output fields are not checked for None value as they will be None generally
                ...
                if form_ok_bool:
                    ...
                    l = float(this_form_in.cleaned_data['l'])  # cylinder width in mm
                    ...
                    cc_obj = CylindricalContact(f, l, v1, v2, e1, e2, d1, d2)
                    b = cc_obj.get_contact_width()  # mm
                    p = cc_obj.get_stress()  # MPa


                    b = remove_trailing_0_fcn(round(b, 3)) # to remove trailing 0s
                    p = remove_trailing_0_fcn(round(p, 0)) # to remove trailing 0s

                    
                    this_form_in = FormIn(request.POST, prefix=prefix_str, initial={'b':b,'p':p,}) # to fill mass in form
    ...
    return render(request, "rw_cylcontact_app/rw_cylcontact.html", {'form_in_lst':form_in_lst,})
