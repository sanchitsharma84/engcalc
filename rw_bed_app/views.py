...
from mechpress import section_mi, bed
import math

@login_required
def rw_bed_fcn(request):
    form_in_lst = []
        ...
        for x in range(nr_rows):
            ...
            if this_form_in.is_valid():
                ...
                if form_ok_bool:
                    ...
                    l = float(this_form_in.cleaned_data['l'])  # udl length in center in mm
                    ...
                    bed_obj = bed.Bed(pf * 1e4, cd / 1e3, l / 1e3, 1, sec_y / 1e3, sec_i / 1e12, wx / 1e3, wy / 1e3, e * 1e6, g * 1e6)
                    
                    sb = bed_obj.get_sb() / 1e6  # bending stress in MPa
                    ss = bed_obj.get_ss() / 1e6  # shear stress in MPa
                    def_m = bed_obj.get_def_b() * 1e3  # deflection due to bending in mm
                    def_s = bed_obj.get_def_s() * 1e3  # deflection due to shear in mm
    ...
    return render(request, "rw_bed_app/rw_bed.html", {'form_in_lst':form_in_lst,})
