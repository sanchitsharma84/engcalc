...
from mechpress import section_mi, slide
import math

@login_required
def rw_slide_fcn(request):
    form_in_lst = []
        ...
        for x in range(nr_rows):
            ...
            if this_form_in.is_valid():
                ...
                if form_ok_bool:
                    ...
                    lr = float(this_form_in.cleaned_data['lr'])  # LR size of slide
                    pc_l = float(this_form_in.cleaned_data['pc_l'])  # % distance under load
                    ...
                    weight = sec_a * lr * 7.85 * 1e-6
                    
                    slide_obj = slide.Slide(pf * 1e4, cd_sus / 1e3, lr / 1e3, pc_l, sec_y / 1e3, sec_i / 1e12, wx / 1e3, wy / 1e3, e * 1e6, g * 1e6)
                    
                    sb = slide_obj.get_sb() / 1e6  # bending stress in MPa
                    ss = slide_obj.get_ss() / 1e6  # shear stress in MPa
                    def_m = slide_obj.get_def_b() * 1e3  # deflection due to bending in mm
                    def_s = slide_obj.get_def_s() * 1e3  # deflection due to shear in mm
                    def_pm = (def_m + def_s) / (cd_sus/1e3)  # total deflection in mm/m
    ...
    return render(request, "rw_slide_app/rw_slide.html", {'form_in_lst':form_in_lst,})
